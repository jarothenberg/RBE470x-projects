...
class TestCharacter(CharacterEntity):
    ...
    def nonWalkableNeighbors(self, world: World, coords): 
        x = coords[0]
        y = coords[1]
        wallCoords = []
        if y+1 < world.height():
            if world.wall_at(x,y+1):
                wallCoords.append((x,y+1))
            if x+1 < world.width() and world.wall_at(x+1,y+1):
                wallCoords.append((x+1,y+1))
            if x-1 >= 0 and world.wall_at(x-1, y+1):
                wallCoords.append((x-1, y+1))
        if y-1 >= 0:
            if world.wall_at(x,y-1):
                wallCoords.append((x,y-1))
            ...
            if x-1 >= 0 and world.wall_at(x-1, y-1):
                wallCoords.append((x-1, y-1))
        if x+1 < world.width() and world.wall_at(x+1, y):
            wallCoords.append((x+1, y)) 
        if x-1 >= 0 and world.wall_at(x-1,y):
            wallCoords.append((x-1,y))
        return wallCoords
